...
def ajout_ennemi(nom,x,y):
    ...
    Ennemi.append(liste)
    cree_sprite(liste[4],liste[5],liste)
    fichier.close()
...
def changeMap():
    ...
    if change:
        ...
        cree_sprite(mon_perso[4],mon_perso[5],mon_perso)
...
def frame():
    ...
    if(not attendre):
        ...
        # print mapx
        verif_jeu()
        canvas.after_idle(canvas.after,35,frame)

def cree_sprite(sprite_pos,nom,entite):
    efface(sprite_pos)
    val_x=entite[2]
    val_y=entite[3]
    fichier = open(nom,'r')
    ...
    for ligne in lignes:
        for i in ligne:
            if i!="0":
                ...
                fig=canvas.create_rectangle(val_x,val_y,val_x+2,val_y+2,fill="",outline="")
                sprite_pos.append(fig)
            val_x+=2.
        val_y+=2
        val_x=entite[2]
...
def mouvement_perso():
    ...
    val_x=mon_perso[2]-val_x
    val_y=mon_perso[3]-val_y
    bouger_sprite(mon_perso[4],mon_perso[5],val_x,val_y)
    teleportation()
    if len(inventaire)>0:
        ramasse_objet()
...
def mouvement_ennemi(mon_ennemi):
    ...
    bloquer_entite(val_x,val_y,mon_ennemi)
    val_x=mon_ennemi[2]-val_x
    val_y=mon_ennemi[3]-val_y
    bouger_sprite(mon_ennemi[4],mon_ennemi[5],val_x,val_y)
...
def vitesse(event):
    ...
    touche=event.keysym
    print canvas.itemconfigure(mon_perso[4][8])
...
def bouger_sprite(sprite_pos,nom,x,y):
    ...
    #             num=0
    #             hexa=""
    #         canvas.move(sprite_pos[cpt],val_x,val_y)
    #         canvas.itemconfig(sprite_pos[cpt],fill=hexa)
    #         cpt+=1
    cree_sprite(mon_perso[4],mon_perso[5],mon_perso)
    fichier.close()
...
def create_HUD():
    ...
    for id in HUD[1]:
        canvas.delete(id)
    HUD=[[],[]]
    HUD[1].append(canvas.create_text(55,30, text = "x" + str(mon_perso[8][1]),fill='white'))
    HUD[1].append(canvas.create_text(150 ,30, text = "x" + str(mon_perso[8][4]),fill='white'))
    HUD[1].append(canvas.create_text(250 ,30, text = "x" + str(mon_perso[8][0]),fill='white'))
    HUD[1].append(canvas.create_text(350 ,30, text = "x" + str(mon_perso[8][5]),fill='white'))
    HUD[1].append(canvas.create_text(450 ,30, text = "\"c\"",fill='white'))
    HUD[1].append(canvas.create_text(550 ,30, text = "\"b\"",fill='white'))
    placerHUD()
...
def sauvegarder():
    ...
    fichier.write("]\n")
    for i in Ennemi:
        mot="["+str(i[2])+","+str(i[3])+","+str(i[9])+","+i[5]+",]\n"
        fichier.write(mot)  
    fichier.close()
...
nommap2="tuto1"
deplacement=0
inventaire=[20,10,True,True,0,0]#[fleche,vie,Epee,Arc,Gold,clef]
